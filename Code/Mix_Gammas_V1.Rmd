...
estimation2.f <- function(dat, para){
        numOfDist <- nrow(para)
        output <- gammamixEM2(dat, k = numOfDist, epsilon = 0.1, verb = T)
        return(output[2:4])
}
...
estimation3.f <- function(dat, para){
        ...
        para.est <- lapply(c(1:numOfDist), function(i){
                print(i)
                nr.gamma(x=unlist(clf.dat[i]), alpha = a.est[i], eps = 0.01, lambda = l.est[i])   # here has issue and what is the output like?
                }) 
