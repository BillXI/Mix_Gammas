...
while(i < B){
  i <- i+1
  s1 <- system.time(tmp.out_1 <- try(suppressWarnings(gammamixEM.new(x=x[,i], mom.start = TRUE, fix.alpha = TRUE, 
                                                                     verb=FALSE, maxit=10000, eps=1e-5)),silent = TRUE))
  s2 <- system.time(tmp.out_2 <- try(suppressWarnings(gammamixEM.new(x=x[,i], mom.start = FALSE, fix.alpha = TRUE, 
                                                                     verb=FALSE, maxit=10000, eps=1e-5)),silent = TRUE))
...
save.image(file="./Result/2018_8_16/14_100.Rd")